...
#pragma region spatial operations
	...
	const int _second_limit_ = (_N_ - (_N_ % 2)) / 2;																						  \
		for (int _n_first_ = -_first_limit_; _n_first_ < (_first_limit_ + (_M_ % 2)); _n_first_++) {				  \
			for (int _n_second_ = -_second_limit_; _n_second_ < (_second_limit_ + (_N_ % 2)); _n_second_++) {		  \
...
#pragma region error checking
	...
	{																							 \
		...
		Launch::Error = cudaGetLastError();															 \
		if(Launch::Error != cudaSuccess) {															 \
			std::cout << "CUDA error at " << #_kernel_ << " - "									 \
			<< cudaGetErrorName(Launch::Error) << ":" << cudaGetErrorString(Launch::Error) << std::endl; \
			abort();													  						 \
			}																					 \
	}		
